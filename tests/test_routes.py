...
class TestProductRoutes(TestCase):
    ...
    def test_update_product_nonexist(self):
        """It should Update an non existing Product"""

        # try update the product
        non_product_id = random.randint(100, 1000)
        test_product = ProductFactory()
        response = self.client.post(BASE_URL, json=test_product.serialize())
        new_product = response.get_json()
        response = self.client.put(f"{BASE_URL}/{non_product_id}", json=new_product)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_update_invalid_product(self):
        """It should try an Update without an Id"""
        # try update the product
        non_product_id = None
        test_product = ProductFactory()
        response = self.client.post(BASE_URL, json=test_product.serialize())
    ...
    def test_query_by_price(self):
        ...
        # check the data just to be sure
        for product in data:
            self.assertEqual(float(product["price"]), float(test_price))
